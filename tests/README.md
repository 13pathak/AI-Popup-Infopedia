...
* **`test-followup-draft.js`**: Issue #40 regression tests for exact draft preservation through session storage and popup restoration, repeated dismissal, cleared/submitted drafts, legacy payloads, and missing inputs. Run independently with `node tests/test-followup-draft.js`.
