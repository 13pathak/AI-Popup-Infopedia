...
- **Direct Pronunciation & IPA Badge:** An inline speaker right next to the looked-up word reads just the word itself (not the whole definition), and a standard IPA transcription (e.g. ɪˈfem.ər.əl) is fetched automatically for single-word lookups — a dictionary affordance for language learners and test prep.
- **Quick Follow-up Chips:** One-click pill buttons above the follow-up input — your own customizable set (e.g. 💡 Simpler, 📝 2 Examples, 🏛️ Etymology) plus up to 3 contextual suggestions the answering model appends to its own reply. The suggestions ride the same request (zero extra API calls) and refresh after every answer; models that skip them simply show your static chips. Both kinds are managed under Options → Follow-up Settings.
